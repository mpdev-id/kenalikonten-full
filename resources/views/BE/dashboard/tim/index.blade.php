      ...
      *Terdapat kesalahan dalam pengisian form. Mohon periksa kembali.
      ...
      <form action="{{ route('dashboard.jenis-tim.create') }}" method="post" enctype="multipart/form-data">
        ...
        <input name="icon" type="file" placeholder="Masukkan link atau tautan konten yang kamu baca" class="w-full rounded-lg bg-[#FFF] py-3 px-2 outline-none placeholder-slate-400 text-slate-[#BDBDBD] text-slate-[16px]" value="{{ old('icon') }}" required>
        @error('icon')
        <span class="text-red-500">*{{ $message }}</span>
        @enderror
        </div>
